...
class Rotate(pt.behaviour.Behaviour):
    ...
    def update(self):
        ...
        self.logger.debug("%s.update()" % self.__class__.__name__)
        self.publisher.publish(Twist(angular=Vector3(z=self.ang_vel))) # Here we are giving the angular velocity to z axis as one
        self.feedback_message = "Rotate {0}".format(self.ang_vel)
    ...
    def terminate(self, new_status):
        ...
        ### YOUR CODE HERE ###
        self.publisher.publish(geometry_msgs.Float32(data =""))
...
class StopMotion(pt.behaviour.Behaviour):
    ...
    def __init__(self, name="stop platform",
                 topic_name="/cmd_vel",
                 ang_vel=0.0):
        # inherit all the class variables from the parent class and make it a behavior
    ...
    def setup(self, **kwargs):
        """Setting up things which generally might require time to prevent delay in the tree initialisation
        """
        self.logger.info("[STOP] setting up rotate behavior")
    ...
    def update(self):
        ...
        self.logger.debug("%s.update()" % self.__class__.__name__)
        self.publisher.publish(Twist(angular=Vector3(z=self.ang_vel))) # Here we are giving the angular velocity to z axis as one
        self.feedback_message = "STOP {0}".format(self.ang_vel)
        return pt.common.Status.RUNNING
        #return.pt.common.Status.SUCCESS
    ...
    def terminate(self, new_status):
        ...
        i.e. when the status changes from RUNNING to SUCCESS or FAILURE
        """
        self.logger.info("[STOP] terminate: publishing zero angular velocity")
...
class BatteryStatus2bb(ptr.subscribers.ToBlackboard):
    ...
    def update(self):
        ...
        ### YOUR CODE HERE ###
        if battery_voltage_topic_name < self.threshold:
    ...
    def terminate(self, new_status):
        ...
        i.e. when the status changes from RUNNING to SUCCESS or FAILURE
        """
        self.logger.info("[STOP] terminate: Battery low")
        
        ### YOUR CODE HERE ###
        self.subscriber.subscribe(battery_voltage_topic_name.Float32)
        self.feedback_message = "cleared"
    '''
